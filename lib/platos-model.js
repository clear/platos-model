...
Platos.create = function (name, collection) {
	var Model = function (properties) {
		...
		if (!Model._meta.forceCollection && this._super && this._super._meta.collection) {
			Model._meta.collection = this._super._meta.collection;
			Model.prototype._meta.collection = this._super._meta.collection;
		}
	};
			...
			if (!_.contains([ "pre", "post", "removePre", "removePost" ], key)) {
				...
				if (_.isFunction(parent.prototype[key])) {
					this.prototype[key] = function () {
						return this._super[key].apply(this, arguments);
					};
				} else {
